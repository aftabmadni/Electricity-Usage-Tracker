...
export const UsageSummarySidebar: React.FC = () => {
  ...
  const daysInMonth = getDaysInCurrentMonth();
    ...
    const usageStats = React.useMemo(() => {
      ...
      const stats = appliances.reduce((acc, app) => {
        const daysSinceAdded = Math.min(currentDay, getDaysSinceCreation(app.createdAt));
              ...
              <SidebarMenuItem key={index}>
                <SidebarMenuButton className="flex-col items-start h-auto py-3">
                  <div className="flex items-center gap-2 w-full mb-1">
                    ...
                    <span className="ml-auto text-xs text-gray-500">{item.subLabel}</span>
                  </div>
